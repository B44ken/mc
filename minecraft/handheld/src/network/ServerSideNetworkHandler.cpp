...
static RakNet::RakNetGUID getLocalGuid(IRakNetInstance* raknetInstance, RakNet::RakPeerInterface* rakPeer) {
	if (rakPeer) return rakPeer->GetMyGUID();
	return RakNet::RakNetGUID((uint64_t)(uintptr_t)raknetInstance);
}
...
void ServerSideNetworkHandler::entityAdded(Entity* e) {
	...
	if(packet != NULL) {
		if (e->isMob() && !e->isPlayer()) {
			redistributePacket(packet, getLocalGuid(raknetInstance, rakPeer));
			delete packet;
		} else {
			raknetInstance->send(packet);
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID& source, LoginPacket* packet)
{
	...
	LoginStatusPacket loginStatusPacket(loginStatus);
	raknetInstance->send(source, loginStatusPacket);
	...
	// send world seed
	{
		...
		raknetInstance->send(source, startGamePacket);
	}
}
...
void ServerSideNetworkHandler::onReady_ClientGeneration(const RakNet::RakNetGUID& source)
{
	...
	// send level info
	SetTimePacket setTimePacket(level->getTime());
	raknetInstance->send(source, setTimePacket);
	...
	for (unsigned int i = 0; i < players.size(); i++) {
		...
		raknetInstance->send(source, addPlayerPacket);

        if (player->getArmorTypeHash()) {
			PlayerArmorEquipmentPacket armorPacket(player);
			raknetInstance->send(source, armorPacket);
        }
	}
	...
	for (unsigned int i = 0; i < level->entities.size(); ++i) {
		...
		Packet* packet = getAddPacketFromEntity(e);
		if(packet != NULL) {
			raknetInstance->send(source, packet);
		}
	}
	...
	*/
	...
	AddPlayerPacket newPlayerPacket(newPlayer);
	redistributePacket(&newPlayerPacket, source);
}
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID& source, RequestChunkPacket* packet)
{
	...
	raknetInstance->send(source, cpacket);
...
void ServerSideNetworkHandler::levelGenerated( Level* level )
{
	...
	if (minecraft->player) {
		minecraft->player->owner = getLocalGuid(raknetInstance, rakPeer);
	}
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID& source, PlayerEquipmentPacket* packet)
{
	...
	Player* player = getPlayer(source);
	if (!player) return;
	if (getLocalGuid(raknetInstance, rakPeer) == player->owner) return;
...
void ServerSideNetworkHandler::levelEvent( Player* source, int type, int x, int y, int z, int data )
{
	...
	redistributePacket(&packet, source? source->owner : getLocalGuid(raknetInstance, rakPeer));
}
